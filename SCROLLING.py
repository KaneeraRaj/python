...
def detect(landmarks,hand):
    ...
    tip,ip=landmarks.landmark[4],landmark.landmarks[3]
...
print("Gesture scroll active open palm-Scroll up, fist-scroll down press'q' to quit")

last=time.time()
while cap.isOpened():
    ok,img=cap.read()
    if not ok :break
    image=cv2.flip(cv2.cvtColor(img,cv2.COLOR_RGB2BGR),1)
    ...
    cv2.putText(img,f"hand:{hand}|Gesture:{gesture}",(10,30),cv2.FONT_HERSHEY_COMPLEX_SMALL,0.7,(255,0,0),2)
